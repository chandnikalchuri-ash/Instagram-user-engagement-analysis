# Instagram-user-engagement-analysis
SQL-based analysis of Instagram user engagement, activity levels, and marketing insights.

---

## 📌 Problem Statement

Analyze Instagram user data to understand engagement patterns, identify high-value users, and generate business insights for marketing strategies.

---

## 📂 Dataset

The dataset includes:
- Users  
- Photos  
- Likes  
- Comments  
- Follows  
- Tags  
- Photo_Tags  

---

## ⚙️ Approach

- Performed data cleaning (NULL & duplicate checks)  
- Used SQL joins and aggregations  
- Applied CTEs and window functions  
- Calculated engagement metrics  
- Segmented users based on activity  

---

## 🏗️ Architecture Overview

The project follows a structured data pipeline:

- Data Source: Users, Photos, Likes, Comments, Follows, Tags
- Data Cleaning: NULL & duplicate checks
- Processing Layer: SQL joins, aggregations, CTEs, window functions
- Analysis Layer: Engagement metrics, user segmentation
- Output: Insights, charts, and business recommendations


---

## 📊 Key Insights

- 26% users are inactive → retention opportunity  
- Top 15–20% users drive most engagement  
- Consistent posting increases engagement  
- Emerging users = growth potential  

---

## 📈 Visual Analysis

### Active vs Inactive Users

This chart show the distribution of users based on their activity levels. A majority of users are active, while a smaller segment remains inactive, indicating potential for re-engagement strategies.

<img width="439" height="339" alt="Screenshot (9)" src="https://github.com/user-attachments/assets/068fb6d7-0b32-49d9-97bb-951adc79a4ec" />


---

### Engagement Rate Analysis  

This chart represents the engagement rate (likes + comments per post) across top users. It highlights that a small group of users generates significantly higher engagement, making them ideal candidates for influencer marketing and targeted campaigns.

<img width="665" height="501" alt="Screenshot (6)" src="https://github.com/user-attachments/assets/f7197aed-6ca2-497e-9b85-03005cdd9255" />


---


### High Engagement Users Insight  

This visualization highlights the importance of targeting high engagement users for marketing campaigns. These users generate maximum interaction and can be leveraged for influencer marketing and brand promotions.

 <img width="671" height="499" alt="Screenshot (7)" src="https://github.com/user-attachments/assets/e1566ff6-0ffc-4232-b3ae-1a2fdb2f415c" />

 
---

## 🔮 Future Improvements

- Build dashboard using Power BI  
- Add real-time data tracking  
- Include ad campaign analysis  
